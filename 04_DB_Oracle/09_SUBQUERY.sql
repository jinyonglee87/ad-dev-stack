...
-- 4. 가장 리뷰 수가 많은 영화 조회
SELECT TITLE, COUNT(REVIEW_ID) FROM REVIEW
JOIN MOVIE USING(MOVIE_ID)
GROUP BY TITLE
ORDER BY COUNT(REVIEW_ID) DESC;

SELECT TITLE FROM (SELECT TITLE, COUNT(REVIEW_ID) FROM REVIEW
JOIN MOVIE USING(MOVIE_ID)
GROUP BY TITLE
ORDER BY COUNT(REVIEW_ID) DESC)
WHERE ROWNUM = 1;
...
SELECT TITLE FROM (SELECT TITLE, COUNT(REVIEW_ID),
DENSE_RANK() OVER(ORDER BY COUNT(REVIEW_ID) DESC)
RANK FROM REVIEW
JOIN MOVIE USING(MOVIE_ID)
GROUP BY TITLE)
...
SELECT TITLE FROM (SELECT TITLE, AVG(RATING), DENSE_RANK() OVER(ORDER BY AVG(RATING) DESC) RANK
FROM REVIEW
JOIN MOVIE USING (MOVIE_ID)
GROUP BY TITLE
HAVING AVG(RATING) > (SELECT AVG(RATING) FROM REVIEW));