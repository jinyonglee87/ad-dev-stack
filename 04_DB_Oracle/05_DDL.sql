...
ALTER TABLE USER_INFO ADD CNAME VARCHAR(20);
...
-- 변경하려는 자료형의 크기보다 이미 큰 값이 존재해서 에러 발생
...
DESC USER_INFO; -- 테이블 description
...
CREATE TABLE DEPT_COPY AS SELECT * FROM DEPARTMENT; -- DUPLICATE
...
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C008363;
...
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C008362 TO DEPT_ID_PK;
...
SELECT * FROM DEPT_COPY; -- renamed to DEPT_TEST
...
DROP TABLE DEPT_TEST; -- dropped DEPT_TEST, thus no longer exist
SELECT * FROM DEPT_TEST;

-- 참조되는 있는 부모 테이블은 함부로 삭제되지 않는다
DROP TABLE BOOK;
...
SELECT * FROM DEPT_COPY;
TRUNCATE TABLE DEPT_COPY; -- 테이블 삭제가 아닌 데이터만 지움