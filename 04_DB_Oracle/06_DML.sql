/*
    ...
    INSERT INTO 테이블명 (SUBQUERY)
     ...
     - 서브쿼리의 결과값이 INSERT 문에 지정된
       테이블 컬럼의 개수와 데이터 타입이 같아야함
*/
...
-- 컬럼 순서를 미리 기재 했기 때문에 데이터 순번에 따라 넣기만 하면 된다
...
INSERT INTO EMP_01 (
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, HIRE_DATE FROM EMPLOYEE
...
/*
    INSERT ALL
    두개 이상의 테이블에 각각 INSERT하는데 동일한 서브쿼리가 사용되는 경우

    INSERT ALL를 이용해 여러 테이블에 한 번에 데이터 삽입이 가능
...
FROM EMPLOYEE
WHERE 1=0; -- 1=0는 틀만 만들어주고 데이터 값은 생략한다
...
INTO EMP_MANAGER (EMP_ID, EMP_NAME, MANAGER_ID)
...
-- EMPLOYEE에서 입사일 기준으로 2000년 1월 1일 이전 사원과
-- 이후 입사한 사원의 테이블을 분리
...
/*
    ...
    WHERE 조건;
     ...
     - SET 절에서 여러 개의 컬럼을 COMMA(,)로 나열해서 값을 동시에 변경할 수 있다.
...
UPDATE USER_02
SET MBTI = 'INTP'
WHERE MBTI IS NULL; -- 사실 조건에서는 PRIMARY KEY로 조건을 거는걸 전제로 한다.
ROLLBACK; --ROLLBACK TO SAVEPOINT 01
